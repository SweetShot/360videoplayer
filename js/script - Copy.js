...
function init2() {
    ...
    var texture = new THREE.Texture( videoImage );
    ...
    var s = "<img id='canvasPlayImg' src='textures/pause.png'>" +
        ...
        "<input type='file' id='openfile'>Open</input>";
    ...
    function update(dt) {
        ...
        // Due to orientation;
        if (oldOrientationEvent && orientationEvent) {
            // abc x                                  nop in motion
            // acb x vertical <=> horizontal          x
            // bac x vertical not working horizontal inverted     good in motion with negatives
            // bca x around like acb
            // cab yup Well enough with negative values ,     nop in motion
            // cba x nope

            var deltaA = THREE.Math.degToRad(orientationEvent.alpha - oldOrientationEvent.alpha);
            var deltaB = THREE.Math.degToRad(orientationEvent.beta - oldOrientationEvent.beta);
            var deltaC = THREE.Math.degToRad(orientationEvent.gamma - oldOrientationEvent.gamma);
            deltaA = THREE.Math.degToRad(orientationEvent.rotationRate.alpha);
            deltaB = THREE.Math.degToRad(orientationEvent.rotationRate.beta);
            deltaC = THREE.Math.degToRad(orientationEvent.rotationRate.gamma);
            var newPos = rotateTargetAroundOrigin(-deltaB, -deltaA, -deltaC, camera.target);
            //console.log(newPos);
            oldOrientationEvent = orientationEvent;
            camera.target.x = newPos[0];
            camera.target.y = newPos[1];
            camera.target.z = newPos[2];
        }
    ...
    window.addEventListener("devicemotion", function(event) {
        if (oldOrientationEvent == null){
            oldOrientationEvent = event;
        }
